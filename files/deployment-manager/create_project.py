...
def generate_config(context):
    ...
    if (context.properties.get('iam-policy-patch') or
        ...
        for api in context.properties['apis']:
            get_iam_policy_dependencies.append(get_api_resource_name(project_id, api))
    ...
    if context.properties.get('bucket-export-settings'):
        ...
        action_dependency = [
            project_id,
            get_api_resource_name(project_id, 'compute.googleapis.com')
        ]
        ...
        if context.properties['bucket-export-settings'].get('create-bucket'):
            ...
            resources.append({
                ...
                'metadata': {
                    'dependsOn': [
                        project_id,
                        get_api_resource_name(project_id, 'storage-component.googleapis.com')
                    ]
                }
            })
...
def is_project_parent_valid(properties):
    """
